...
def batchify_with_label(input_batch_list, gpu, volatile_flag=False):
    ...
    pad_chars = [chars[idx] + [[0] for _ in range(max_seq_len - len(chars[idx]))]
...
def train(data, save_model_dir, seg=True):
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            if not instance:
                continue
            gaz_list, batch_word, batch_biword, batch_wordlen, batch_wordrecover, \
            batch_char, batch_charlen, batch_charrecover, batch_label, mask = batchify_with_label(instance, data.HP_gpu)

            instance_count += 1
            with torch.enable_grad():

                loss, tag_seq = model.neg_log_likelihood_loss(gaz_list, batch_word, batch_biword, batch_wordlen,
                                                              batch_char,
